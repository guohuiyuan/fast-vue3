...
// 获取歌单所有数据
export function getPlaylistAllDetail(data: { id: string; limit?: number; offset?: number }) {
  const query = qs.stringify({
    ...data,
    timestamp: Date.now(),
  });
...
// 更新歌单标签
export function updatePlaylistTags(data: { id: string; tags: string }) {
  const query = qs.stringify({
    ...data,
    timestamp: Date.now(),
  });
...
// 编辑歌单
export function updatePlayListInfo(data: { id: string; name: string; tags: string; desc: string }) {
  const query = qs.stringify({
    id: data.id,
    name: data.name,
    tags: data.tags,
...
// 更新歌单封面
export function updatePlayListCover(file: File, imgSize: number, id: string) {
...
// 歌单评论
export function getPlaylistComment(data: { id: string; limit?: number; offset?: number; before?: string }) {
